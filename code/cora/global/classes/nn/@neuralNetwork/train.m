...
%               .volume_heuritsic: {'interval','f-radius'}
...
if verbose
    ...
    fprintf('Neural Network Training Parameters:\n');
    fprintf('-----------------------------------\n');
    fprintf(" - Training method: '%s'\n",options.nn.train.method);
    fprintf(' - Optimizer: %s\n',optim.print());
    fprintf(' - Training epochs: %d\n',maxEpoch);
    fprintf(' - Mini-batche size: %d\n',miniBatchSize);
    fprintf(' - Shuffle data: %s\n',options.nn.train.shuffle_data);
    if strcmp(options.nn.train.method,'point')
    else
        fprintf(' - (max) Training noise: %.2e\n',maxNoise);
        if strcmp(options.nn.train.method,'madry')
            fprintf(' - Number of PGD iterations: %i\n', ...
                options.nn.train.pgd_iterations);
        elseif strcmp(options.nn.train.method,'gowal')
            fprintf(' - kappa (gowal): %.2e\n',options.nn.train.kappa);
        elseif strcmp(options.nn.train.method,'set')
            fprintf(" - Volume heuristic for training: '%s'\n", ...
                options.nn.train.volume_heuritsic);
            fprintf(' - Weight for volume heuristic in loss (tau): %.2e\n', ...
                options.nn.train.tau);
            fprintf(' - Type of input generators: %s\n', ...
                options.nn.train.init_gens);
            fprintf(' - Number of generators: %d\n', ...
                options.nn.train.num_init_gens);
        end
    end
    fprintf(' - Early stopping after non-decr. validation steps: %i\n', ...
...
for epoch=1:maxEpoch % epoch
    ...
    for i=1:miniBatchSize:max(1,N - mod(N,miniBatchSize)) % training iterations
        ...
        if verbose && ismember(iter,printIter) 
            trainTime = toc;
            aux_printTrainingIteration(iter,epoch,trainTime,printIter, ...
                valIter,loss,options)
        end
    ...
    elseif strcmp(options.nn.train.volume_heuritsic,'f-radius')
        ...
        throw(CORAerror('CORA:wrongFieldValue', ...
            sprintf("Unsported volume heuristic: %s; Only supported values " + ...
                "for 'volume_heuritsic' are 'interval' and 'f-radius'!!",...
                    options.nn.train.volume_heuritsic)));
    end
...
function aux_printTrainingIteration(iter,epoch,trainTime,printIter, ...
    valIter,loss,options)
% Specify values to print
printCols = {'Epoch','Iteration','Training Time (hh:mm:ss)',...
...
colFormats = {"d","d","s",".4e",".4e",".4e",".4e"};
printColsRule = "|=" + strjoin(cellfun(@(s) ...
    repmat('=',[1,strlength(s)]),printCols,...
    'UniformOutput',false),'=|=') + "=|\n";
% Print table header
if iter == 1
    fprintf(printColsRule);
    fprintf("| " + strjoin(printCols,' | ') + " |\n");
    fprintf(printColsRule);
end
% Set values for each column
valIterIdx = find(valIter == iter);
trainTimeVec = [0 0 0 0 0 trainTime];
colVals = {epoch,iter,datetime(trainTimeVec,'Format','HH:mm:ss'),...
    loss.center(iter),loss.vol(iter),loss.total(iter),...
    loss.valTotal(valIterIdx)};
fprintf("| " + strjoin(cellfun(@sprintf,"%" + ...
    cellfun(@strlength,printCols) + ...
    colFormats,colVals,'UniformOutput',false),' | ') + " |\n");
% Print a rule after the last iteration
if iter == printIter(end) 
    fprintf(printColsRule);
end
end
