...
if setTrainFields
    for i=1:size(defaultTrainFields, 1)
        field = defaultTrainFields{i, 1};
        if ~isfield(options.nn.train, field)
            fieldValue = defaultTrainFields{i, 2};
            if isa(fieldValue, "function_handle")
                fieldValue = fieldValue(options);
            end
            options.nn.train.(field) = fieldValue;
        end
    end
end
...
if CHECKS_ENABLED
    ...
    if setTrainFields
        ...
        aux_checkFieldStr(options.nn.train,'method', ...
            {'point','set','madry','gowal','trades','sabr'},structName);
        % 'set' training parameters
        aux_checkFieldStr(options.nn.train,'volume_heuritsic', ...
